...
class office extends base
{
     ...
     * @return string
     */
    function get_horizontal($position = '')
    {
     ...
     * @return string
     */
    function get_vertical($position = '')
    {
        $position = empty($position) ? $this->vertical : $position;
        switch ($position) {
            case "top":
     ...
     * @return bool
     */
    function set_column_width($columns)
    {
        $before = "A";
     ...
     * @param $title
     */
    function set_sheet_title($title)
    {
     ...
     * @return array[]
     */
    function set_row_style($level = 'empty')
    {
     ...
     * @return array
     */
    function setting_conversion($config)
    {
        $return_config = [];
     ...
     * @return array[]
     */
    function get_default_title_setting($color, $conf = [])
    {
        switch ($color) {
            case "green":
                $conf = [
     ...
     * @return bool
     */
    function check_excel_column($title, $data)
    {
     ...
     * @param $data
     * @param array $titles
     * @param null $title_style
     * @return Spreadsheet
     * @throws \PhpOffice\PhpSpreadsheet\Exception
     */
    function export($data, $titles = [], $title_style = NULL)
    {
        ...
        if (!empty($titles)) {
            ...
            foreach ($titles as $key => $title) {
                ...
                if (is_numeric($key)) {
                    ...
                    // $conf[$after] = $this->get_default_title_setting('customize', $title);   // 默认自动加上粗体，14px字号
                }
     ...
     */
    function save_export($data, $titles = [], $title_style = NULL)
    {
        ...
        $save_file = date('Ymd') . '-' . getordernum('T') . '.xlsx';
        create((BASE_DIR . $save_path));
        $write->save((BASE_DIR . $save_path . DS . $save_file));

        return SITEROOTURL . $save_path . DS . $save_file;
    }
     ...
     * @throws \PhpOffice\PhpSpreadsheet\Writer\Exception
     */
    function download_export($data, $titles = [], $title_style = NULL)
    {
    ...
    function import($file, $sheets = [])
    {
        ...
        $loader = $reader->load($file['tmp_name']);
